...
class SaleOrder(models.Model):
        ...
        for line in self.order_line.filtered(lambda l: l.product_id.type == 'service'):
            ...
            if line_timesheets:
                ...
                updated_lines.append(
                    f"{line.product_id.name}: {line_hours:.1f} hours = {line_days:.2f} days")

        if updated_lines:
            message = _(
                "Delivered quantities updated from timesheets:\n") + "\n".join(updated_lines)
    ...
    def action_create_invoice_from_timesheets(self):
        ...
        if not self.timesheet_hours_total:
            raise UserError(
                _('No timesheet entries found for this sales order.'))
